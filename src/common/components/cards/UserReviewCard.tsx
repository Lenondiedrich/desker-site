...
export default function UserReviewCard({ stars = 3, quote, image, name, role }: UserReviewType) {
  const starClass = (star: number) => (star <= stars ? "text-main" : "text-neutral-700");
  ...
  return (
    <div className=" flex flex-col justify-between rounded-md border border-neutral-800 bg-neutral-900/50 p-8 shadow-sm ">
      <div>
        ...
        <p className="my-4 mb-0 text-base font-normal leading-relaxed tracking-wide text-gray-400">{quote}</p>
      </div>
      <div className="mt-6 flex items-center gap-6 ">
        ...
        <div>
          ...
          <p className="text-xs leading-relaxed tracking-wide text-gray-400">{role}</p>
        </div>
      </div>
    </div>
  );
