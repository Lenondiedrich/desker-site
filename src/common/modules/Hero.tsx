import { ANIMATE_BUTTON, ANIMATE_SUBTITLE } from "@/common/animations/Animations";
...
import NotificationTag, { NotificationType } from "@/common/components/tags/NotificationTag";
...
export default function Hero({ notfication, buttons, title, description, image }: HeroType) {
  return (
    <Container>
      <div className="my-6 mx-auto w-full max-w-4xl pt-4 text-center lg:pt-14">
        <NotificationTag text={notfication?.text} tag={notfication?.tag} href={notfication?.href} button={notfication?.button} />
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-4 sm:mt-6 sm:gap-6">
          {buttons &&
            buttons.map((button, index) => (
              ...
              >
                <NextButton href={button.href} title={button.title} type={button.type} />
              </motion.div>
            ))}
        </div>
      </div>
      <div className="mt-10 flex items-center justify-center overflow-hidden sm:mt-14 sm:px-10 md:px-0">
        <NextImage src={image.src} width={image.width} className="" priority sizes="(max-width: 768px) 70vw, 100vw" />
      </div>
    </Container>
  );
}
