...
import UserReviewCard, { UserReviewType } from "@/common/components/cards/UserReviewCard";
...
export default function UserReviews({ kicker, title, buttons, userReviews }: UserReviewSectionType) {
  return (
    <>
      ...
      <InnerContainer className="text-center">
        ...
        <div className="mt-10 mb-20 flex flex-wrap justify-center gap-6">
          {buttons && buttons.map((button, index) => <NextButton {...button} key={index} />)}
        </div>
      </InnerContainer>
      <Container>
        <div className="my-10 grid gap-10 sm:grid-cols-2 lg:grid-cols-3">
          {userReviews && userReviews.map((userReview, index) => <UserReviewCard {...userReview} key={index} />)}
        </div>
      </Container>
    </>
  );
